...
public class CaseMutationsInOuterQueryBlock {
	...
	public static void generateDataForKillingCaseMutationsInOuterQueryBlock(GenerateCVC1 cvc) throws Exception{
		...
		logger.log(Level.INFO,"GENERATE DATA FOR KILLING CASE CONDITION MUTATIONS IN OUTER QUERY BLOCK");
		logger.log(Level.INFO,"\n----------------------------------\n");
		...
		if(ccMap != null){
			if(ccMap.containsKey(1)){
				generateDataForCaseInProjectedColumns(cvc);
			}
			...
			//If case condition is in Where clause, add the constraints to conjunts and call generate dataset for conjunts
				generateDataForWherePredicate(cvc);
		...
		try{
			...
			for(int i=0; i < (selectionConds.size()-1); i++){
				...
				/** Generate data set to kill each mutation*/
						...
						cvc.getConstraints().add(GenerateConstraintsForCaseConditions.getConstraintsInCaseStatement(cvc,cvc.getOuterBlock(),sc,caseConditionCompleted));
